...
- **index.html**：入口页。视口 ≥1024px（Mac/桌面）时自动跳转「查看总览」；否则（手机）自动跳转「填写今日」。需手动选择时可打开 `index.html?choose`。
- **schedule_edit.html**：今日编辑页（PWA 主入口），填写 7:00～23:00 日程，可保存到本地、导出 CSV。支持「添加到主屏幕/程序坞」当轻量 app 用。
- **schedule.html**：总览页（只读），年度/月度切换查看已写入 `entries.csv` 的日程。首次打开使用构建时注入的数据；**「刷新」按钮**会从底层数仓 `../entries.csv` 拉取最新数据并重绘（需通过 HTTP 打开，如 `schedule_log_serve.sh`）；若以 file:// 打开则刷新仍为整页重载。
